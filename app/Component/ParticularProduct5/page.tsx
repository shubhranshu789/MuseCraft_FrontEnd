...
function ProductContent() {
    ...
    const sizes = [
        { id: 'size-5x7', size: '5 × 7"', price: 199 },
        { id: 'size-6x6', size: '6 × 6"', price: 269 },
        { id: 'size-6x12', size: '6 × 12"', price: 299 },
        { id: 'size-8x8', size: '8 × 8"', price: 299 },
        { id: 'size-8x10', size: '8 × 10"', price: 299 },
        { id: 'size-8x12', size: '8 × 12"', price: 349 },
        { id: 'size-12x18', size: '12 × 18"', price: 649 }
    ];

    // State using ID instead of price
    const [selectedSizeId, setSelectedSizeId] = useState(sizes[0].id);


    const [isInsideOpen, setIsInsideOpen] = useState(true);
    const [isLoveitOpen, setIsLoveitOpen] = useState(false);

    const insideItems = [inside1, inside2, inside3, inside4, inside5, inside6].filter(Boolean);
    const loveitItems = [loveit1, loveit2, loveit3, loveit4].filter(Boolean);

    const hasInsideItems = insideItems.length > 0;
    const hasLoveitItems = loveitItems.length > 0;

    // Don't render if no items
    if (!hasInsideItems && !hasLoveitItems) {
        return null;
    }
    ...
    const basePrice = Number(searchParams.get('price')) || 1899;
    ...
    //     return selectedSize;
    // };
    const finalPrice = sizes.find(s => s.id === selectedSizeId)?.price || sizes[0].price;
    ...
    const handleBuyNow = () => {
        ...
        const params = new URLSearchParams({
            ...
            id: product.id,
            image: product.image,
            title: `${product.title} - ${selectedSize.size}`,
            price: finalPrice.toString(),
            description: product.description,
            quantity: '1',
            sizeId: selectedSize.id // Optional: pass ID too for tracking
        });
